    ...
    if (supabase) {
        try {
            const supabaseOrderData = {
                ...
                // created_at is handled by default now() in Supabase
            };
            ...
            if (supabaseError) {
                ...
                // Store the returned Supabase UUID in the Firestore document for future updates
        ...
        unsubscribe = onSnapshot(riderOrdersCol, (snapshot) => {
            snapshot.docChanges().forEach((change) => {
                if (change.type === "modified") {
                    ...
                    // Update the main Firestore database directly from here
                    ...
                    // Callback for any UI components that might still be listening (optional)
