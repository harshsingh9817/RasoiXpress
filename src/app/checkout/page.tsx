...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    if (!tempOrderId) {
        toast({ title: "Checkout Error", description: "No order specified.", variant: "destructive" });
        router.replace('/');
        return;
    }
    loadPageData();
  }, [isAuthenticated, isAuthLoading, user, getCartItemCount, router, showSuccessScreen, loadPageData, isOrderingAllowed, setIsTimeGateDialogOpen, tempOrderId, toast]);
  ...
  }, [tempOrderId, isFinalizing, showSuccessScreen, user]);
  ...
  const handlePlaceOrder = async (e: FormEvent) => {
    e.preventDefault();
    if (!user || !selectedAddressId || !tempOrderId) {
    ...
    try {
      ...
      const paymentObject = new window.Razorpay({
        ...
        handler: (response: any) => {
            ...
            // The webhook will handle moving the order, so we just show success
            setTimeout(() => {
                clearCart();
